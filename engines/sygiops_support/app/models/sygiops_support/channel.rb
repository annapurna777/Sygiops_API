...
=end
    ...
    def self.fetch
      channels = SygiopsSupport::Channel.where('active = ? AND area LIKE ?', true, '%::Account')
      channels.each(&:fetch)
    end
...
=end
    ...
    def fetch(force = false)
      ...
      begin
        driver_class    = SygiopsSupport::Channel::Driver::Imap
        ...
        error = "Can't use Channel::Driver::#{adapter}: #{e.inspect}"
        logger.error error
        logger.error e
...
=end
    ...
    def self.driver_class(adapter)
      ...
      require_dependency "sygiops_support/channel/driver/#{adapter.to_filename}"

      "SygiopsSupport::Channel::Driver::#{adapter.to_classname}".constantize
    end
