...
=end
  ...
  def self.build(attr, notification = false)
    ...
    # set organization
    organization = "Sygitech"#Setting.get('organization')
    if organization
...
=end
  ...
  def self.html_complete_check(html)
    ...
    html_email_body = File.read(Rails.root.join('engines/sygiops_support/app/views/mailer/application_wrapper.html.erb').to_s)

    html_email_body.gsub!('###html_email_css_font###', "font-family:'Helvetica Neue', Helvetica, Arial, Geneva, sans-serif; font-size: 12px;")
