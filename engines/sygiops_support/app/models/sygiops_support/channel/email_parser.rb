  ...
  class Channel::EmailParser
    ...
    def _process(channel, msg)
      ...
      Transaction.execute(interface_handle: "#{original_interface_handle}.postmaster") do
        ...
        # get sender user
        session_user_id = 1#mail['x-zammad-session-user-id'.to_sym]
        if !session_user_id
          raise 'No x-zammad-session-user-id, no sender set!'
        end

        session_user = User.find(1)#User.lookup(id: session_user_id)
        if !session_user
        ...
        if !ticket
          ...
          customer = User.find_by(email: mail[:from_email])
          if customer.nil?
            name = mail[:from_display_name].split(" ")
            customer = User.create!(login: mail[:from_email],firstname: name[0].to_s,lastname: name[1].to_s,email: mail[:from_email],active: true ,role_id: Role.find_by(name: "Customer").id)
          end
          organization = Organization.find_by(name: mail[:from_domain])
          # organization_id = nil
          sla = Sla.find_by(default_create: true)
          if organization.nil?
            organization = Organization.create!(name: mail[:from_domain].split(".")[0] ,domain: mail[:from_domain],active: true,sla_id: Sla.find_by(default_create: true)&.id, timezone_name: nil)
          end
          ...
          ticket = Ticket.new(
            ...
            owner_id:    User.find_by(active: false).id,
      ...
      reply_mail = {
        recipient: User.find_by(email: mail[:from_email]),
        subject: "Thanks for your enquiry (#{mail[:subject]}) [Ticket##{ticket.number}]",
        content_type: 'text/html',
        body: '<div>Your request <b>('+"Ticket##{ticket.number}"+')</b> has been received and will be reviewed by our support staff.</div><br/><br/><div>Your Sygiops Support Team</div><br/><div><i><a href=\"https://www.sygitech.com\">Sygiops Support</a>, your customer support system</i></div>',
        message_id: mail[:message_id],
        references: mail[:message_id]
      }

      Delayed::Job.enqueue(SendEmailNotification.new(reply_mail))
    ...
    def message_header_hash(mail)
      ...
      # imported_fields = mail.header.fields.map { |f| [f.name.downcase, f.encode("UTF-8")] }.to_h
