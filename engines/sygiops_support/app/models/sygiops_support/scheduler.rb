module SygiopsSupport
 ...
 def self.threads
   ...
   begin
     ...
     Rails.logger.error "cleanup error"
   ...
   begin
     ...
     Rails.logger.error "worker error"
     Rails.logger.error e
     Rails.logger.error e.backtrace
   end
 ...
 def self.skip_job?(job)
   Rails.logger.info "job to skip: #{job}"
   ...
   if !thread.respond_to?(:status)
     ...
     puts "Invalid thread stored for job '#{job.name}' (#{job.method}): #{thread.inspect}. Deleting and resting job."
   ...
   if status.present?
     ...
     puts "Running job thread for '#{job.name}' (#{job.method}) status is: #{status}"
     return true
   end
   ...
   puts "Job thread terminated #{how} found for '#{job.name}' (#{job.method}). This should not happen. Please report."
   @@jobs_started.delete(job.id)
   false
 end
 ...
 def self.cleanup_delayed_jobs(after)
   ::Mixin::StartFinishLogger.log_start_finish(:info, "Cleanup of left over locked delayed jobs #{after}") do

     Delayed::Job.where('updated_at < ?', after).where.not(locked_at: nil).each do |job|
       ::Mixin::StartFinishLogger.log_start_finish(:info, "Checking left over delayed job #{job.inspect}") do
         cleanup_delayed(job)
       end
     end
   end
 ...
 def self.cleanup_delayed(job)
   ...
   puts  "#{action} locked delayed job: #{job_name}"
 end
 ...
 def self.cleanup_import_jobs(after)
   Mixin::StartFinishLogger.log_start_finish(:info, "Cleanup of left over import jobs #{after}") do
   ...
   Thread.new do
     ...
     if job.period && job.period <= 5.minutes
       ...
       loop do
         loop_count += 1
         _start_job(job)
         job = Scheduler.find_by(id: job.id)
