require_dependency 'sygiops_support/store/object'
require_dependency 'sygiops_support/store/file'
...
=end
  ...
  def self.add(data)
    ...
    store_object = Store::Object.find_or_create_by(name: data['object'])
...
=end
  ...
  def self.list(data)
    # binding.pry
    # search
    store_object_id = Store::Object.find_by(name: data[:object])
    stores = Store.where(store_object_id: store_object_id, o_id: data[:o_id].to_i).order(created_at: :asc)
    stores
  end
...
=end
  ...
  def generate_previews
    return true if false#Setting.get('import_mode')
