module SygiopsSupport
  class Ticket < ApplicationRecord
    ...
    has_many      :articles,               class_name: 'SygiopsSupport::Ticket::Article', dependent: :destroy, inverse_of: :ticket
    belongs_to    :state,                  class_name: 'SygiopsSupport::Ticket::State', optional: true
    belongs_to    :priority,               class_name: 'SygiopsSupport::Ticket::Priority', optional: true
    belongs_to    :owner,                  class_name: 'SygiopsSupport::User', optional: true
    belongs_to    :customer,               class_name: 'SygiopsSupport::User', optional: true
    belongs_to    :create_article_type,    class_name: 'SygiopsSupport::Ticket::Article::Type', optional: true
    belongs_to    :create_article_sender,  class_name: 'SygiopsSupport::Ticket::Article::Sender', optional: true
    ...
    attr_accessor :callback_loop
     ...
     def self.notify_ticket_created
      broadcast_ticket = {}
      broadcast_ticket["ticket"] = self.as_json
      broadcast_ticket["customer"] = User.find_by(id: customer_id).email
      ActionCable.server.broadcast("notification_channel",broadcast_ticket)
    end

    def self.notify_sla_breach
      Rails.logger.info "******************notify_sla_breach********************"
      begin
        where('first_response_escalation_at <= ? OR escalation_at <= ?', Time.now.utc + 30.minutes,Time.now.utc + 30.minutes).find_each(batch_size: 500) do |ticket|
          # next if (!ticket.preferences["sla_id"] || ticket.preferences["escalation_disabled"])
          if ticket.first_response_at == nil && !ticket.response_warning
            ticket.update(response_warning: true)
            Delayed::Job.enqueue(SendNotification.new({data_object: ticket, msg: "Warning response sla breach" , code: "101" , type: "response sla breach warning"}))
          end

          if ticket.first_response_escalation_at && !ticket.response_breach && (ticket.first_response_escalation_at <= Time.now.utc)
            ticket.update(response_breach: true)
            Delayed::Job.enqueue(SendNotification.new({data_object: ticket, msg: "Notification response sla breached" , code: "102" , type: "response sla breached"}))
          end

          if ticket.escalation_at && !ticket.resolution_warning && (ticket.escalation_at <= Time.now.utc + 30.minutes)
            ticket.update(resolution_warning: true)
            Delayed::Job.enqueue(SendNotification.new({data_object: ticket, msg: "Warning resolution sla breach" , code: "103" , type: "resolution sla breach warning"}))
          end

          if ticket.escalation_at && !ticket.resolution_breach && (ticket.escalation_at <= Time.now.utc)
            ticket.update(resolution_breach: true)
            Delayed::Job.enqueue(SendNotification.new({data_object: ticket, msg: "Notification resolution sla breached" , code: "104" , type: "resolution sla breached"}))
          end
        end
      rescue => e
    ...
    def self.filter_tickets(category,option)
      result = {"about" => "something"}
      if category == "organization"
        if option == "none"
          result["about"] = "none"
          Organization.all.each do |organization|
            result[organization.name] = Ticket.where(organization_id: organization.id).map{ |t| t.as_json.merge(priority: t.priority.name, customer: t.customer.firstname+" "+t.customer.lastname,state: t.state.name)}.as_json
          end
        else
          organization = Organization.find_by(id: option.to_i)
          result[organization.name] = Ticket.where(organization_id: organization.id).map{ |t| t.as_json.merge(priority: t.priority.name, customer: t.customer.firstname+" "+t.customer.lastname,state: t.state.name)}.as_json
        end
        ...
        if option == "none"
          ...
          Ticket::State.all.each do |state|
            result[state.name] = Ticket.where(state_id: state.id).map{ |t| t.as_json.merge(priority: t.priority.name, customer: t.customer.firstname+" "+t.customer.lastname,state: t.state.name)}.as_json
          end
        else
          state = Ticket::State.find_by(id: option.to_i)
          result[state.name] = Ticket.where(state_id: state.id).map{ |t| t.as_json.merge(priority: t.priority.name, customer: t.customer.firstname+" "+t.customer.lastname,state: t.state.name)}.as_json
        end
      elsif category == "priority"
        if option == "none"
          result["about"] = "none"
          Ticket::Priority.all.each do |priority|
            result[priority.name] = Ticket.where(priority_id: priority.id).map{ |t| t.as_json.merge(priority: t.priority.name, customer: t.customer.firstname+" "+t.customer.lastname,state: t.state.name)}.as_json
          end
        else
          priority = Ticket::Priority.find_by(id: option.to_i)
          result[priority.name] = Ticket.where(priority_id: priority.id).map{ |t| t.as_json.merge(priority: t.priority.name, customer: t.customer.firstname+" "+t.customer.lastname,state: t.state.name)}.as_json
        end
      elsif category == "type"
        if option == "none"
          result["about"] = "none"
          Ticket::Type.all.each do |type|
            result[type.name] = Ticket.where(type_id: type.id).map{ |t| t.as_json.merge(priority: t.priority.name, customer: t.customer.firstname+" "+t.customer.lastname,state: t.state.name)}.as_json
          end
        else
          type = Ticket::Type.find_by(id: option.to_i)
          result[type.name] = Ticket.where(type_id: type.id).map{ |t| t.as_json.merge(priority: t.priority.name, customer: t.customer.firstname+" "+t.customer.lastname,state: t.state.name)}.as_json
        end
        ...
        if option == "none"
          ...
          User.all.each do |customer|
            result["#{customer.firstname} #{customer.lastname}"] = Ticket.where(customer_id: customer.id).map{ |t| t.as_json.merge(priority: t.priority.name, customer: t.customer.firstname+" "+t.customer.lastname,state: t.state.name)}.as_json
          end
        else
          customer = User.find_by(id: option.to_i)
          result[customer.name] = Ticket.where(customer_id: customer.id).map{ |t| t.as_json.merge(priority: t.priority.name, customer: t.customer.firstname+" "+t.customer.lastname,state: t.state.name)}.as_json
        end
      elsif category == "created_at"
        if option == "none"
          result["about"] = "none"
          tickets = Ticket.all.map{ |t| t.attributes.merge(priority: t.priority.name, customer: t.customer.firstname+" "+t.customer.lastname,state: t.state.name)}
           result =  tickets.group_by { |c| c["created_at"].to_date }
           result = result.as_json
        else
          date = Date.parse(option)
          result[Date.parse(option)] = Ticket.where(created_at: date.midnight..date.end_of_day).map{ |t| t.as_json.merge(priority: t.priority.name, customer: t.customer.firstname+" "+t.customer.lastname,state: t.state.name)}.as_json
        end
      end
      result
    end

    # def notify_resolution_sla_breach
    #     where('escalation_at <= ?', Time.now.utc + 30.minutes).find_each(batch_size: 500) do |ticket|
    #     end
    # end
    ...
    def self.selector2sql(selectors, options = {})
      ...
      selectors.each_key do |attribute|
        ...
        if selector[0] == 'customer'
          tables += ',sygiops_support_ users sygiops_support_customers'
          query += 'sygiops_support_tickets.customer_id = sygiops_support_customers.id'
        elsif selector[0] == 'organization'
          tables += ', sygiops_support_organizations'
          query += 'sygiops_support_tickets.organization_id = sygiops_support_organizations.id'
        elsif selector[0] == 'owner'
          tables += ', sygiops_support_users sygiops_support_owners'
          query += 'sygiops_support_tickets.owner_id = sygiops_support_owners.id'
        elsif selector[0] == 'article'
          tables += ', sygiops_support_ticket_articles sygiops_support_articles'
          query += 'sygiops_support_tickets.id = sygiops_support_articles.ticket_id'
        elsif selector[0] == 'ticket_state'
          tables += ', sygiops_support_ticket_states'
          query += 'sygiops_support_tickets.state_id = sygiops_support_ticket_states.id'
        else
      ...
      selectors.each do |attribute, selector_raw|
        ...
        if selector['operator'].include?('in working time')
          ...
          biz = Calendar.find_by(id: selector['value'])&.biz
          next if biz.blank?
    ...
    def reset_pending_time
      ...
      current_state      = Ticket::State.find_by(id: state_id)
      current_state_type = Ticket::StateType.find_by(id: current_state.state_type_id)
    ...
    def build_sms_recipients_list(value, article)
      ...
      .map { |user_or_id| User.find_by(id: user_or_id) }
